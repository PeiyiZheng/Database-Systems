...
class StorageFile:
  ...
  def __init__(self, **kwargs):
    ...
    if os.path.exists(self.filePath):
      if mode in ["update", "truncate"]:
        openMode    = "r+b" if mode == "update" else "w+b"
        self.file   = open(self.filePath, openMode)
        self.header = FileHeader.fromFile(self.file)

        # Initialize the free pages set here??
        for (pageId, pageHeader) in self.headers():
          if pageHeader.hasFreeTuple():
  ...
  def readPageHeader(self, pageId):
    if self.validPageId(pageId) == True:
      ...
      tempPage = self.pageClass()(pageId=  PageId(0,0), buffer = bytes(self.pageSize()), schema = self.schema())
      headerBytes = self.file.read(tempPage.header.headerSize())
      buffer = io.BytesIO(headerBytes)

      pageHeader = self.pageClass().headerClass.unpack(buffer.getbuffer())
      return pageHeader
    else:
  ...
  def writePageHeader(self, page):
    if self.validPageId(page.pageId) == True:
  ...
  def readPage(self, pageId, page):
    if self.validPageId(pageId) == True:
      offset = self.pageOffset(pageId)
      self.file.seek(offset)
      bytesNum = self.file.readinto(page)

      if bytesNum == self.pageSize():
        newPage = self.pageClass().unpack(pageId, page)

        # It is a new and free page
        if newPage.header.hasFreeTuple():
          self.freePages.add(newPage.pageId)
        return newPage
      else:
        # Incomplete Page
        return None
  ...
  def allocatePage(self):
    lastPageId = self.pageId(self.numPages())
    newPage = self.pageClass()(pageId = lastPageId, buffer = bytes(self.pageSize()), schema = self.schema())
    self.writePage(newPage)
    self.file.flush()
  ...
  def insertTuple(self, tupleData):
    ...
    self.bufferPool.updatePage(page.pageId, page.pack())
    if page.header.hasFreeTuple() == False:
      self.freePages.discard(page.pageId)
  ...
  # This function is not given in the code base but it will be called in unit.py
  def numTuples(self):
    cnt = 0
    for hearder in self.headers():
      cnt += hearder[1].numTuples()
    return cnt
