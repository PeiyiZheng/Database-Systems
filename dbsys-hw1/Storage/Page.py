...
class PageHeader:
  ...
  def usedSpace(self):
    # We need to make sure that any tuples that are removed
    # must shift any subsequent tuples to preserve data contiguity
  ...
  def nextFreeTuple(self):
    if self.hasFreeTuple():
      nextTuple = self.freeSpaceOffset
  ...
  def nextTupleRange(self):
    # Index starts from 0, so the number of tuples is the index of next tuple
...
class Page(BytesIO):
  ...
  def __init__(self, **kwargs):
    ...
    if buffer:
      ...
      # Do we need to add any code here?
  ...
  def getTuple(self, tupleId):
    if 0 <= tupleId.tupleIndex < self.header.numTuples():
  ...
  def deleteTuple(self, tupleId):
    ...
    buffer = self.getbuffer()
    offset = self.header.headerSize() + tupleId.tupleIndex * self.header.tupleSize
    subsequentLength = self.header.freeSpaceOffset - (offset + self.header.tupleSize)

    buffer[offset : offset + subsequentLength] = buffer[offset + self.header.tupleSize : self.header.freeSpaceOffset]
